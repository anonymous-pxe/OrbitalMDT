...
function build_rocket_tab(fig)
    ...
    // Delta-V
    uicontrol(fig, 'style', 'text', 'string', 'Total ΔV (km/s):', ...
    ...
    uicontrol(fig, 'style', 'pushbutton', ...
        'string', '🚀 CALCULATE MASS BUDGET', ...
        'position', [x_panel, y_base+465, panel_w, 35], ...
        'fontsize', 12, 'fontweight', 'bold', ...
        'background', [0.8, 0.2, 0.2], 'foreground', [1, 1, 1], ...
        'callback', 'cb_calculate_rocket()', ...
        'tag', 'content_rk_calculate');
    
    // ---- Results ----
    uicontrol(fig, 'style', 'text', 'string', '■ MASS BUDGET RESULTS', ...
        'position', [x_panel, y_base+435, panel_w, 22], ...
        'fontsize', 11, 'fontweight', 'bold', 'horizontalalignment', 'left', ...
        'foreground', [0.7, 0.1, 0.1], 'tag', 'content_rk_res_title');
    
    res_labels = ['Exhaust Velocity:  —       '; ...
                  'Mass Ratio:        —       '; ...
                  'Payload Mass:      —       '; ...
                  'Propellant Mass:   —       '; ...
                  'Total Init Mass:   —       '; ...
    ...
    for k = 1:7
        uicontrol(fig, 'style', 'text', ...
            'string', res_labels(k,:), ...
            'position', [x_panel, y_base+435-k*23, panel_w, 20], ...
            'fontsize', 9, 'horizontalalignment', 'left', ...
            'fontname', 'Consolas', ...
            'tag', 'content_rk_result_' + string(k));
    end
    
    // ---- ΔV Budget Breakdown ----
    uicontrol(fig, 'style', 'text', 'string', '■ ΔV BUDGET BREAKDOWN', ...
        'position', [x_panel, y_base+260, panel_w, 22], ...
        'fontsize', 11, 'fontweight', 'bold', 'horizontalalignment', 'left', ...
    ...
    for k = 1:5
        uicontrol(fig, 'style', 'text', ...
            'string', '', ...
            'position', [x_panel, y_base+260-k*22, panel_w, 20], ...
            'fontsize', 9, 'horizontalalignment', 'left', ...
            'fontname', 'Consolas', ...
            'tag', 'content_rk_budget_' + string(k));
    end
...
function cb_rocket_preset()
    // Apply mission ΔV preset
    preset = get(findobj('tag', 'content_rk_preset'), 'value');
    
    missions = ['Custom', 'LEO', 'GEO', 'Moon', 'Mars', 'Venus', 'Jupiter'];
    ...
    if preset > 1 then
        ...
        result = mission_dv_budget(missions(preset));
        for k = 1:min(5, length(result.segments))
            set(findobj('tag', 'content_rk_budget_' + string(k)), 'string', ...
                msprintf('%s: %.2f km/s', result.segments(k), result.dvs(k)));
        end
        for k = (length(result.segments)+1):5
            set(findobj('tag', 'content_rk_budget_' + string(k)), 'string', '');
        end
    end
endfunction
...
function cb_calculate_rocket()
    // Calculate rocket equation mass budget
    ...
    update_status(msprintf('Rocket equation: %.0f kg payload needs %.0f kg propellant (%.1f%% prop fraction)', ...
